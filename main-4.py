...
def get_top_animes(limit=10):
    """Eng ko'p so'ralgan (anime ko'rish + qism so'rash birga hisoblanadi) animelar
    ro'yxatini qaytaradi: [(name, code, count), ...]."""
    conn = get_db()
    c = conn.cursor()
    c.execute("""
        ...
        WHERE al.event_type IN ('anime_view', 'episode_request')
...
async def handle_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(update.effective_user.id) and context.user_data.get("awaiting_channel_custom_days"):
