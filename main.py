...
async def _create_media_database(bot, db_path, backup_path):
    ...
    try:
        ...
        # Video fayllar Telegram Bot API'ning 20MB get_file() cheklovi tufayli
        # deyarli har doim yuklanmaydi ("File is too big") — shuning uchun ularni
        # yuklashga umuman urinmaymiz, vaqt behuda ketmasligi uchun. Restore paytida
        # bu videolar eski file_id orqali ishlashda davom etadi (baza o'zida saqlaydi).
        skipped_video_count = len(
            backup_conn.execute(
        ...
        backup_conn.commit()
        return media_count, failed_count + skipped_video_count
    finally:
        source_conn.close()
        backup_conn.close()
